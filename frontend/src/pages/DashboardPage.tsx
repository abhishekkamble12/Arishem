...
export const DashboardPage: React.FC = () => {
  ...
  const filteredFiles = files.filter(file => {
    const matchesSearch = file.s3_key.toLowerCase().includes(fileSearchQuery.toLowerCase());
    const matchesFilter = selectedSourceFilter ? file.s3_key === selectedSourceFilter : true;
        ...
        <div className={`md:col-span-7 flex flex-col h-full min-h-0 bg-dark-900/35 border border-dark-800/40 rounded-2xl p-4 overflow-hidden relative ${activeTab === 'chat' ? 'flex' : 'hidden md:flex'
          ...
          <div className="flex items-center justify-between border-b border-dark-800/60 pb-3.5 mb-4 flex-shrink-0">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h2 className="font-bold text-white text-sm">Bedrock RAG Agent</h2>
                <p className="text-[10px] text-dark-500">Claude 3.5 Sonnet + Titan Embeddings</p>
              </div>
            </div>
            {messages.length > 0 && (
              <button
              ...
              <div className="space-y-4">
                ...
                {isQuerying && (
                  <div className="flex flex-col items-start max-w-[80%] animate-pulse-subtle">
                    <div className="flex items-center space-x-1.5 text-[10px] text-dark-500 mb-1 px-1">
                      <span>Claude Agent</span><span>•</span><span>Thinking...</span>
                    </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
            {isFetchingFiles && files.length === 0 ? (
              ...
              filteredFiles.map((file) => {
                const isCited = selectedSourceFilter === file.s3_key;
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center space-x-2 truncate">
                        ...
                        <span className="text-xs font-bold text-dark-100 truncate" title={file.s3_key}>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {canDelete && (
                          <button
                            onClick={() => handleDelete(file.s3_key)}
                            disabled={isDeleting}
